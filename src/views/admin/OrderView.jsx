...
import _, { upperFirst, clone, map, omit, padStart } from "lodash";
...
class OrderView extends Component {
    ...
    requestChanges = orderId => {
        this.setState({ rejectionModalOpen: true });
    };
    render() {
        ...
        return (
            <div>
                <RejectOrderModal
                    open={state.rejectionModalOpen}
                    orderId={orderId}
                    handleClose={this.closeRejectionModal}
                />
                    ...
                    <div style={{ marginTop: 10 }}>
                        {orderType === "kye" ? (
                            ...
                            <>
                                ...
                                <p>
                                    Registration No: {order.registrationNumber}
                                </p>
                ...
                {order.status !== "pending" &&
                    order.status !== "rejected" && order.status !== "changes requested" &&
                    (order.orderType === "kyc" ? (
...
function RejectOrderModal({ open, handleClose, orderId }) {
    const [description, updateDescription] = useState("");
    function sendRequest() {
        if (description.length < 3) return;
        return requestOrderChanges(orderId, description)
            .then(res => handleClose())
            .catch(error => {
                console.log("Error: ", error);
                alert(
                    "There was an error requesting the changes, please try again later"
                );
            });
    }
    return (
        ...
        >
            ...
            <DialogContent>
                <DialogContentText>
                    Please write a short descriptiont to help the client make
                    the necessary changes for this order to meet your
                    requirements.
                </DialogContentText>
                <TextField
                    autoFocus
        ...
        <Paper style={{ padding: "1em", marginTop: 15 }}>
            ...
            <Grid container style={{ marginTop: 5 }}>
                <Grid item xs={12} style={{ paddingLeft: 3, paddingRight: 3 }}>
                    <p style={{ fontSize: 18 }}>
                        Attach any supporting documents such as an in depth
                        report, certificates, scanned Identification,
                        statements, etc.
                    </p>
                </Grid>
                <Grid
                ...
                >
                    {state.attachmentsURL && state.attachmentsURL.length > 3 && (
                        <Button
                            onClick={() => downloadAttachment(state)}
                            color="primary"
                        >
...
function StakeholdersAnalysis({ order }) {
    ...
    function saveStakeholderAnalysis() {
        ...
        const dates = _.values(state.shareholders).map(
            holder => holder.idExpiry
        );
        _.values(state.directors).forEach(director =>
            dates.push(director.idExpiry)
        );
        if (!isValidDate(dates)) {
            return alert("Please validate the dates entered");
        }

        const emptyFields = _.values(state.directors).filter(
            director => !isCompleteForm(director)
        );
        emptyFields.concat(
            _.values(state.directors).filter(
                director => !isCompleteForm(director)
            )
        );

        console.log("Empties: ", emptyFields);
                    ...
                    <Grid item xs={12} sm={12} md={3} style={styles.inputs}>
                        {Object.keys(state.shareholders).length > 1 && (
                            ...
                            >
                                <Button
                                    onClick={() =>
                                        removeEmbeded("shareholders", key)
                                    }
                                    size="small"
                                    color="secondary"
                                >
...
function CompanyAnalysis({ order }) {
    ...
    function saveCompanyAnalysis() {
        ...
        if (!isValidDate(state.incorporationDate)) {
            return alert("Please validate the dates entered");
        }
