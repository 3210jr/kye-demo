...
import _, { map, clone, keys } from "lodash";
...
function GapsReports({ order, type, snackbar, toggleSnackBar }) {
    ...
    function saveGapsReport(resultKey) {
        ...
        if (!isValidDate([from, to])) {
            return alert("Please verify you have entered valid dates");
        }
        if (
            comments.length < 5 ||
        ...
        persistOrderEmbeddedResults(order.id, type, resultKey, {
            ...
            .then(res => {
                toggleSnackBar({
                    message: "Gap Reports updated successfully!"
                });
            })
            .catch(error => {
                toggleSnackBar({
                    message:
                        "Error. There was an error updating the Gap report."
                });
